...
struct mp4_avcC_box *mp4_read_avcC_box(FILE *f)
{
    ...
    ptr->numOfPictureParameterSets = read_uint16_lit(f);
...
struct mp4_stsd_box mp4_read_stsd_box(FILE *f, int size)  //level 8
{
    ...
    for(int i =0; i < box.number_of_entries; ++i){
        ...
        }else if(box.sample_description[i].type == (('a'<<24)|('v'<<16)|('c'<<8)|('1'))){
...
struct mp4_stsc_box mp4_read_stsc_box(FILE *f, int size)  //level 8
{
    ...
    box.entry_list = new mp4_list_t[box.map_amount];
    printf("\t\t\t\t\t\tfirst trunk:\tsamples-per-thunk:\tsample-description-ID\n");
    for(int i = 0; i < box.map_amount; ++i){
        box.entry_list[i].first_chunk_num = read_uint32_lit(f);
        box.entry_list[i].sample_amount_in_cur_table = read_uint32_lit(f);
        box.entry_list[i].sample_description_id = read_uint32_lit(f);
        
        printf("\t\t\t\t\t\t%13d", box.entry_list[i].first_chunk_num);
        printf("\t%13d", box.entry_list[i].sample_amount_in_cur_table);
        printf("\t%13d\n", box.entry_list[i].sample_description_id);
    }
    //delete box.entry_list;
    return box;
}
...
struct mp4_stco_box mp4_read_stco_box(FILE *f, int size)
{
    ...
    for(int i = 0 ; i < box.chunk_offset_amount; ++i){
        ...
        //printf("%8u ", box.chunk_offset_from_file_begin[i]);
    }
...
void print_trak(const struct mp4_trak_box * trak)
{
    ...
    printf("\t\t\twidth: %u\n",trak->tkhd.width);
    printf("\t\t\theight: [%u].[%u]\n",trak->tkhd.height & 0xffff0000 >> 16,trak->tkhd.height & 0xffff);
	...
	{
        printf("\t\t\t\t\t\t%13d",trak->mdia.minf.stbl.stsc.entry_list[i].first_chunk_num);
        printf("\t%13d",trak->mdia.minf.stbl.stsc.entry_list[i].sample_amount_in_cur_table);
        printf("\t%13d\n",trak->mdia.minf.stbl.stsc.entry_list[i].sample_description_id);
    }
    printf("\n");
...
uint32_t get_sample_num_in_cur_chunk(const struct mp4_stsc_box & box,const uint32_t chunk_index) 
{
    ...
    for(int i = 0; i < box.map_amount; ++i) 
	{
        if(i+1 == box.map_amount)
		{
            ...
            sample_num_in_cur_chunk_  = box.entry_list[i].sample_amount_in_cur_table;
        }

        if(chunk_index+1 >= box.entry_list[i].first_chunk_num
           && chunk_index+1 < box.entry_list[i+1].first_chunk_num)
		{
            sample_num_in_cur_chunk_ =  box.entry_list[i].sample_amount_in_cur_table;
            break;
        }
...
uint32_t get_sample_index(const struct mp4_stsc_box &box, const uint32_t chunk_index)
{
    /*chunk   samples   id
      1         4        1
      4         6        1
    ...
    for(int i = 0; i < box.map_amount; ++i) 
	{
        //chunk_index [0,end),  first_chunk_num [1,end)
        if( box.entry_list[i + 1].first_chunk_num-1  <=  chunk_index)
		{
               uint32_t chunk_amount_which_has_same_amount_of_samples
                = box.entry_list[i + 1].first_chunk_num
                - box.entry_list[i].first_chunk_num + 1;
            
            index += box.entry_list[i].sample_amount_in_cur_table *
                chunk_amount_which_has_same_amount_of_samples;
            
        } 
		else
		{
            uint32_t chunk_amount_which_has_same_amount_of_samples
                = chunk_index+1 - box.entry_list[i].first_chunk_num;
...
uint32_t get_sample_size(const struct mp4_stsz_box &box,  const uint32_t index)  //begin from 0
{
    for(int i = 0; i < box.table_size; ++i) 
	{
        if( index == i)
...
void copy_chunk_data(FILE *fin,  const struct mp4_stbl_box &box, const uint32_t chunk_index, FILE *fout)
{
    fseek(fin, box.stco.chunk_offset_from_file_begin[chunk_index], SEEK_SET);
    
    uint32_t sample_num_in_cur_chunk_ = get_sample_num_in_cur_chunk(box.stsc, chunk_index);  //@a mark
    uint32_t sample_index_ = get_sample_index(box.stsc, chunk_index);

    for(int i = 0; i < sample_num_in_cur_chunk_; ++i) 
	{
        uint32_t sample_size_ = get_sample_size(box.stsz, sample_index_);
...
void make_file(FILE *fin, const struct mp4_trak_box * trak, FILE *fout)
{
